...
public class AppdevapiApplication {
  ...
  CommandLineRunner run(UserService userService, PermissionLevelService permissionLevelService,
                        ...
                        RatingServiceImpl ratingService, WishlistService wishlistService,
    ...
    return args -> {
      if (orderItemService.getAllOrderItems().size() < 1) {
        ...
        userService.save(new UserDto("Arne", "Arnesen", "arne@arnesen.arn", "1234", "Arneveien 13",
                "C/O Anette Anettessen", "1440", "Bergen", "Norge", "13647980"));
        userService.save(
                new UserDto("Per", "Person", "per@person.per", "1234", "Gamle Persgate 72a", "6011",
                        "Ålesund", "Norge", "46537894"));
        ...
        productService.addProductFromDto(new ProductDto("Heather tea",
                "From Norwegian mountains. Gathered carefully before the bees to hold the honey taste. Rich in vitamins. Local produce",
                "Norwegian mountains", 200, 10, 0, "Tea"));
        productService.addProductFromDto(new ProductDto("Linden blossom tea",
                "Classic Latvian tea. Helps against laziness. Use 100C water (not typical for herbal teas). Gathered in summer 2021",
                "Classic Latvian tea", 200, 5, 0, "Tea"));
        productService.addProductFromDto(new ProductDto("Sencha 50g",
                "Japanese green tea. Green leaves. Available in Aug-Sep season only",
                "Japanese green tea", 100, 20, 0, "Tea"));
        productService.addProductFromDto(new ProductDto("Sencha 500g",
                "Japanese green tea. Green leaves. Available in Aug-Sep season only",
                "Japanese green tea", 800, 36, 0, "Tea"));
        productService.addProductFromDto(new ProductDto("Mug",
                "Classic mug. Made from Brazilian clay. Hot-friendly - comfortable to hold even when the water is hot. Handy handle",
                "Brazilian clay", 120, 70, 0, "Accessories"));
        ...
        for (Product product : productService.getAllProducts()) {
          ...
          if (imageResource != null) {
            ...
            MultipartFile multipartFile =
                    new MockMultipartFile("product" + index, index + ".jpeg", "image/jpeg",
                            imageStream);
        ...
        ratingService.addRating(new Rating(userService.findAll().get(0), "Geir",
                productService.getProductByName("Heather tea"), 4, "ok"));
        ratingService.addRating(new Rating(userService.findAll().get(0), "Geir",
                productService.getProductByName("Linden blossom tea"), 5, "Best tea ever"));
        ratingService.addRating(new Rating(userService.findAll().get(0), "Geir",
                productService.getProductByName("Sencha 50g"), 2, "yum"));
        ratingService.addRating(new Rating(userService.findAll().get(0), "Geir",
                productService.getProductByName("Sencha 50g"), 2, "more quantity of yum"));
        ratingService.addRating(new Rating(userService.findAll().get(0), "Geir",
                productService.getProductByName("Mug"), 5, "Nice mugs"));
        ratingService.addRating(new Rating(userService.findAll().get(1), "Per",
                productService.getProductByName("Linden blossom tea"), 5, "Great tea"));
        ratingService.addRating(new Rating(userService.findAll().get(1), "Per",
                productService.getProductByName("Sencha 50g"), 2, "This was weird"));
        ratingService.addRating(new Rating(userService.findAll().get(1), "Per",
                productService.getProductByName("Sencha 50g"), 2, "This was a lot of weird"));
        ratingService.addRating(new Rating(userService.findAll().get(1), "Per",
                productService.getProductByName("Mug"), 5, "Great big mugs"));
        ...
        shoppingSessionService.addShoppingSession(
                new ShoppingSession(userService.findAll().get(0)));
        shoppingSessionService.addShoppingSession(
                new ShoppingSession(userService.findAll().get(1)));
        shoppingSessionService.addShoppingSession(
                new ShoppingSession(userService.findAll().get(2)));
        shoppingSessionService.addShoppingSession(
                new ShoppingSession(userService.findAll().get(3)));
        shoppingSessionService.addShoppingSession(
                new ShoppingSession(userService.findAll().get(4)));
