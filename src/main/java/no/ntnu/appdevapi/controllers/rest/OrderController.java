...
public class OrderController {
  ...
  public ResponseEntity<String> add() throws MessagingException {
    ...
    if (getUser() != null && !getShoppingSession().getCart().isEmpty() &&
            getUser() == getShoppingSession().getUser()) {
      ...
      Iterable<OrderItem> orderItems =
              this.convertCartItemSetToOrderItemSet(orderDetails, getShoppingSession().getCart());
      this.orderItemService.addAllOrderItems(orderItems);

      //Send order confirmation email and empty shopping session.
      applicationEventPublisher.publishEvent(
              new CompleteOrderEvent(getUser(), orderDetails, orderItems));
   ...
   */
  @PutMapping("/{orderId}")
  public ResponseEntity<String> update(@PathVariable long orderId,
    ...
    if (null != orderDetails && null != old) {
      ...
      for (int i = 0; i < itemIds.length(); i++) {
        ...
        items.add(new OrderItem(old,
                productService.getProduct(Long.valueOf((Integer) itemIds.get(i))),
   ...
   */
  private Set<OrderItem> convertCartItemSetToOrderItemSet(OrderDetails orderDetails,
  ...
  private OrderDetails convertShoppingSessionToOrderDetails(ShoppingSession shoppingSession) {
    return new OrderDetails(shoppingSession.getUser(), shoppingSession.getTotal(),
            shoppingSession.getQuantity());
  }
  ...
  private boolean doesOrderExist(long orderId, User user) {
    ...
    return orderDetails != null && orderDetails.getId() == orderId &&
            this.orderDetailsService.getOrderDetails(orderId) != null &&
            orderDetails.getUser() == user;
  }
