...
public class CompleteOrderListener implements ApplicationListener<CompleteOrderEvent> {
   ...
   */
  private String createOrderConfirmationMailContent(User user, OrderDetails orderDetails,
                                                    Iterable<OrderItem> orderItems) {
    StringBuilder content = new StringBuilder(
            "<h1>Dear " + user.getFirstName() + " " + user.getLastName() +
                    ",</h1><p>Your order has been received and will swiftly be processed. Thank you for supporting Urban Infusion</p>" +
                    "<br> <br>" +
                    "<table style=' margin: 0 auto;' width='700'>\n" +
                    "  <tr style='display:flex;'>\n" +
                    "    <td width='250'>" +
                    "     <p style='font-weight: bold; line-height: 0;'>Order number:</p><p>" +
                    orderDetails.getId() + "</p><br>" +
                    "     <p style='font-weight: bold; line-height: 0;'>Order date:</p><p>" +
                    orderDetails.getCreatedAt().toLocalDate() + "</p><br>" +
                    ...
                    "    </td>\n" +
                    "    <td width='250'>" +
                    "     <p style='font-weight: bold; line-height: 0;'>Full name:</p><p>" +
                    orderDetails.getUser().getFirstName() + " " +
                    orderDetails.getUser().getLastName() + "</p><br>" +
                    "     <p style='font-weight: bold; line-height: 0;'>Email-address:</p><p>" +
                    orderDetails.getUser().getEmail() + "</p><br>" +
                    "     <p style='font-weight: bold; line-height: 0;'>Phone-number:</p><p>" +
                    orderDetails.getUser().getAddress().getPhone() + "</p><br>" +
                    "     <p style='font-weight: bold; line-height: 0;'>Home-address:</p><p>" +
                    orderDetails.getUser().getAddress().getAddressLine() + "</p><br>" +
                    "    </td>\n" +
                    "  </tr>\n" +
                    "</table>" +
                    "<br> <br>" +
    ...
    for (OrderItem orderItem : orderItems) {
      content.append("<tr>" +
                      "<td style='border-bottom: 1px solid #ddd;'>").append(orderItem.getProduct().getId())
              .append("</td>")
              .append("<td style='border-bottom: 1px solid #ddd;'>")
              .append(orderItem.getProduct().getName()).append("</td>")
              .append("<td style='border-bottom: 1px solid #ddd;'>").append(orderItem.getQuantity())
              .append("</td>")
              .append("<td style='border-bottom: 1px solid #ddd;'>").append(orderItem.getTotal())
              .append(",-").append("</td>")
              .append("</tr>");
    }
    content.append(
                    "<tr><td></td><td></td><td></td><td style='line-height: 2;'><strong>Total: </strong>")
            .append(orderDetails.getTotal()).append(",-").append("</td></tr>").append("</table>");
