 ...
 */
@Component
public class CompleteRegistrationListener
        implements ApplicationListener<CompleteRegistrationEvent> {
  ...
  private String createConfirmRegistrationMailContent(User user) {
    String confirmationUrl =
            "/api/confirm-registration?token=" + verificationTokenService.getTokenFromUser(user);
    String link = ("localhost:8080" + confirmationUrl);


    return "<h1 style='text-align: center;  color: #58555F; font-family: Montserrat, sans-serif; font-size: 24px'>Hi " +
            user.getFirstName() + "! Welcome to Urban Infusion!</h1>" +
            "<br>" +
            ...
            "' style='padding: 8px 16px; color: #fff; box-shadow: 0 5px 10px rgba(0, 0, 0, 0.15);" +
